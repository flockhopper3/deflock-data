...
4. [`upload.sh`](cameras/upload.sh) uploads three datasets to the R2 data bucket with `Cache-Control: public, max-age=3600` (1-hour cache) and `x-last-updated` / `x-feature-count` / `x-source` metadata

| R2 key | Contents |
|--------|----------|
| `cameras.geojson.gz` | US + Canada merged (deduped, sorted by OSM id) — consumed by the tile pipeline and `data.dontgetflocked.com` |
| `cameras-us.geojson.gz` | US only |
| `cameras-ca.geojson.gz` | Canada only |
...
The tile build (`build-tiles.yml`, hourly at :23) picks up the fresh `cameras.geojson.gz` the same hour.
